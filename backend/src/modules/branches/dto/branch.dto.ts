...
export const createBranchSchema = z.object({
  ...
  addressLine1: z.string().trim().min(1).max(160).optional().default('—'),
  addressLine2: z.string().trim().max(160).optional(),
  city: z.string().trim().min(1).max(80).optional().default('—'),
  state: z.string().trim().min(1).max(80).optional().default('Tamil Nadu'),
  country: z.string().trim().min(1).max(80).default('India'),
  postalCode: z.string().trim().min(3).max(16).optional().default('000000'),
